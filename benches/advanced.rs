...
fn bench_advanced(c: &mut Criterion) {
    ...
    bench_rgb_case(
        &mut group,
        "direct_pattern_control_rgb_256_iters2",
        &fixture,
        width,
        height,
        |buffer| direct_pattern_control_in_place(buffer, 2),
    );
    bench_rgb_case(
        &mut group,
        "hierarchical_colorant_dbs_rgb_256_iters2",
        &fixture,
        width,
        height,
        |buffer| hierarchical_colorant_dbs_in_place(buffer, 2),
    );
    group.finish();
    ...
    bench_gray_case(
        &mut group,
        "clustered_dot_direct_multibit_search_gray_32_iters4_levels4",
        &fixture,
        width,
        height,
        |buffer| clustered_dot_direct_multibit_search_in_place(buffer, 4, 4),
    );
    group.finish();
}
